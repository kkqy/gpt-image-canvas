...
const zhMessages = {
  ...
  galleryBatchDeleteBody: ({ count }: { count: number }) =>
    `将从 Gallery、生成历史和画布移除 ${count} 张选中图片，并删除对应本地文件和资产记录。云端备份不会删除。`,
  ...
  galleryConfirmDeleteBody: ({ excerpt }: { excerpt: string }) =>
    `将从 Gallery、生成历史和画布移除“${excerpt}”，并删除对应本地文件和资产记录。云端备份不会删除。`,
...
const enMessages: I18nMessages = {
  ...
  galleryBatchDeleteBody: ({ count }) =>
    `Remove ${count} selected images from Gallery, generation history, and canvas, and delete their local files and asset records. Cloud backups will not be deleted.`,
  ...
  galleryConfirmDeleteBody: ({ excerpt }) =>
    `Remove "${excerpt}" from Gallery, generation history, and canvas, and delete its local file and asset record. Cloud backups will not be deleted.`,
