...
function waitForGenerationPoll(signal: AbortSignal, delayMs: number): Promise<void> {
  if (signal.aborted) {
...
export function App() {
  ...
  async function pollGenerationRecord(requestId: number, generationId: string): Promise<GenerationRecord> {
    const task = activeGenerationsRef.current.get(requestId);
    if (!task) {
      throw new DOMException("Generation task is no longer active.", "AbortError");
    }

    while (true) {
      await waitForGenerationPoll(task.controller.signal, 1400);
  ...
  function removeGalleryOutputFromHistory(deletedItems: DeletedGalleryItem[]): void {
    ...
    const removedCanvasCount = removeGalleryItemsFromCanvas(deletedItems);
    if (removedCanvasCount > 0) {
      setGenerationMessage(t("galleryCanvasRemoved", { count: removedCanvasCount }));
    }
  }
  ...
  async function cancelGeneration(requestId: number): Promise<void> {
    ...
    task.controller.abort();
    const cancelledRecord = task.serverGenerationId ? await cancelServerGeneration(task.serverGenerationId).catch(() => undefined) : undefined;
    const editor = editorRef.current;
    if (editor) {
      deleteLoadingGenerationPlaceholders(editor, task.placeholderSet);
    }
    ...
    setGenerationHistory((history) =>
      history.map((record) =>
        record.id === task.temporaryRecordId || record.id === task.serverGenerationId
  ...
  async function cancelServerGeneration(generationId: string): Promise<GenerationRecord | undefined> {
    ...
    const body = (await response.json()) as unknown;
    return isGenerationResponse(body) ? body.record : undefined;
  }
